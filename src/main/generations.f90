...
program generations
  ...
  INTEGER , DIMENSION(:,:), allocatable :: pedigree
  INTEGER , DIMENSION(:  ), allocatable :: sires,dams
  INTEGER   :: npairs
  INTEGER   :: nsires,ndams,nanim,ngen,noff
...
call askInteger ( ngen, ' number of generations' )
...
write(*,*)' get into iniyiailse genotypes',genstart
...
!maxloci=0
!maxblock=0
! If genstart=={1,2}, this block of code sets the position based on the number of markers and chromlength; Freq = 0
...
do
  ...
  call poissonProb(chrL,maxchiasma,val2) !get prob from poisson dist
  val1=val1+val2 ! get cumulative prob
...
!    WRITE(*,*) ' segregating loci (total / MAF >= 0.05) ',k,igen
  ...
  !---------------------------------------------
    ...
    ELSEIF(eliminationType==1) then
      ...
      enddo
    ELSEIF(eliminationType==2) then
      call  SORTDX(nloci,MAF,MAFsorted) ! todo: what is this
      eliminated(:)=1
      j=0
      k=nloci
      do WHILE(j < nkept)
